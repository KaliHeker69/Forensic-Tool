...
## New Plugin Integration: `windows.ldrmodules`

The engine now ingests `ldrmodules` output as a first-class data source (instead of treating it like `dlllist`).

### Expected input

- `jsonl/ldrmodules.jsonl` (recommended)
- `json/ldrmodules.json` (also supported)
...
### New analysis and correlation behavior

- `INJ007` detection rule added:
  - Flags `False/False/False` entries (hidden from all PEB lists).
  - Flags partially unlinked entries (e.g., `InMem=False` while other lists indicate loaded state).
  - Correlates `ldrmodules` base addresses with `malfind` ranges for high-confidence reflective injection findings.

### Report generation updates

- HTML report deep-analysis now includes **LdrModules Correlation** section with:
  - Hidden module count
  - Unlinked module count
  - Overlap count with `malfind`
  - Per-module triage table (state, base, path, overlap)
- JSON report now includes `parsed_stats` with:
  - `ldrmodules_count`
  - `ldr_hidden_count`
  - `ldr_unlinked_count`
  - `malfind_count`

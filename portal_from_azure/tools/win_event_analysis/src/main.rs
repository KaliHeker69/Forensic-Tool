...
Any hostnames added to the critical_systems.txt file will have all alerts above low increased by one level with a maximum of emergency level.";
        ...
        if stored_static.forensic_summary_flag {
            let cmd_name = Action::get_action_name(stored_static.config.action.as_ref());
            write_color_buffer(
                &BufferWriter::stdout(ColorChoice::Always),
                get_writable_color(
                    Some(Color::Rgb(255, 175, 0)),
                ...
                &format!("Generating {}", cmd_name.replace('-', " ")),
                true,
            )
            .ok();
            println!();
        ...
        match &stored_static.config.action.as_ref().unwrap() {
            ...
            a if Action::is_forensic_summary(Some(a)) => {
                if let Some(path) = &stored_static.output_path
                    && !stored_static.output_option.as_ref().unwrap().clobber
                    && utils::check_file_expect_not_exist(
                        path.as_path(),
    ...
    fn analysis_files(
        ...
        if matches!(
            ...
            let cmd_name = Action::get_action_name(stored_static.config.action.as_ref());
            // Use a leaked string for the wait message lifetime
            wait_message = Box::leak(format!("Currently scanning for {}. Please wait.", cmd_name).into_boxed_str());
        } else if stored_static.forensic_report_flag {
            wait_message = "Currently scanning for forensic report. Please wait.";
        }
        ...
        *STORED_STATIC.write().unwrap() = Some(stored_static.clone());
        ...
        let is_show_progress = stored_static.output_path.is_some()
            || stored_static.forensic_summary_flag
            || stored_static.forensic_report_flag
            || matches!(
                stored_static.config.action.as_ref().unwrap(),
                Action::ConfigCriticalSystems(_)
                    | Action::PivotKeywordsList(_)
        ...
        if stored_static.metrics_flag {
            ...
            tl.forensic_summary_dsp_msg(stored_static)
        } else if stored_static.forensic_report_flag {
            tl.forensic_report_dsp_msg(stored_static)
    ...
    fn check_is_valid_args_num(&self, action: Option<&Action>) -> bool {
        match action.as_ref().unwrap() {
            ...
            a if Action::is_forensic_summary(Some(a)) => env::args().len() != 2,
            Action::ForensicReport(_) => env::args().len() != 2,
            _ => true,
        }
    }
}

...
pub struct StoredStatic {
    ...
    pub forensic_summary_flag: bool,
    pub forensic_report_flag: bool,
...
impl StoredStatic {
    ...
    pub fn create_static_data(input_config: Option<Config>) -> StoredStatic {
        ...
        let common_options = match &input_config.as_ref().unwrap().action {
            ...
            Some(Action::ServiceSummary(opt)) | Some(Action::DriverSummary(opt))
            | Some(Action::CrashSummary(opt)) | Some(Action::AccountChanges(opt))
            | Some(Action::GroupChanges(opt)) | Some(Action::PrivilegeUseSummary(opt))
            | Some(Action::PasswordChanges(opt)) | Some(Action::FirewallSummary(opt))
            | Some(Action::RdpSummary(opt)) | Some(Action::ShareAccessSummary(opt))
            | Some(Action::SoftwareInstallSummary(opt)) | Some(Action::WindowsUpdateSummary(opt))
            | Some(Action::KerberosSummary(opt)) | Some(Action::FailedLogonDetail(opt))
            | Some(Action::LogonTypeBreakdown(opt)) | Some(Action::AuditPolicyChanges(opt))
            | Some(Action::LogCleared(opt)) | Some(Action::ObjectAccessSummary(opt))
            | Some(Action::ScheduledTaskSummary(opt)) | Some(Action::ProcessExecutionSummary(opt))
            | Some(Action::ScheduledPersistenceSummary(opt)) | Some(Action::LateralMovementSummary(opt))
            | Some(Action::AccountLockoutSummary(opt)) | Some(Action::PolicyTamperingSummary(opt))
            | Some(Action::RebootShutdownSummary(opt)) | Some(Action::SuspiciousEncodingSummary(opt)) => {
                opt.common_options
            }
            Some(Action::ForensicReport(opt)) => {
                opt.common_options
            }
            None => CommonOptions {
                no_color: false,
        ...
        let output_path = match &input_config.as_ref().unwrap().action {
            ...
            Some(Action::ServiceSummary(opt)) => opt.output.as_ref(),
            Some(Action::DriverSummary(opt)) => opt.output.as_ref(),
            Some(Action::CrashSummary(opt)) => opt.output.as_ref(),
            Some(Action::AccountChanges(opt)) => opt.output.as_ref(),
            Some(Action::GroupChanges(opt)) => opt.output.as_ref(),
            Some(Action::PrivilegeUseSummary(opt)) => opt.output.as_ref(),
            Some(Action::PasswordChanges(opt)) => opt.output.as_ref(),
            Some(Action::FirewallSummary(opt)) => opt.output.as_ref(),
            Some(Action::RdpSummary(opt)) => opt.output.as_ref(),
            Some(Action::ShareAccessSummary(opt)) => opt.output.as_ref(),
            Some(Action::SoftwareInstallSummary(opt)) => opt.output.as_ref(),
            Some(Action::WindowsUpdateSummary(opt)) => opt.output.as_ref(),
            Some(Action::KerberosSummary(opt)) => opt.output.as_ref(),
            Some(Action::FailedLogonDetail(opt)) => opt.output.as_ref(),
            Some(Action::LogonTypeBreakdown(opt)) => opt.output.as_ref(),
            Some(Action::AuditPolicyChanges(opt)) => opt.output.as_ref(),
            Some(Action::LogCleared(opt)) => opt.output.as_ref(),
            Some(Action::ObjectAccessSummary(opt)) => opt.output.as_ref(),
            Some(Action::ScheduledTaskSummary(opt)) => opt.output.as_ref(),
            Some(Action::ProcessExecutionSummary(opt)) => opt.output.as_ref(),
            Some(Action::ScheduledPersistenceSummary(opt)) => opt.output.as_ref(),
            Some(Action::LateralMovementSummary(opt)) => opt.output.as_ref(),
            Some(Action::AccountLockoutSummary(opt)) => opt.output.as_ref(),
            Some(Action::PolicyTamperingSummary(opt)) => opt.output.as_ref(),
            Some(Action::RebootShutdownSummary(opt)) => opt.output.as_ref(),
            Some(Action::SuspiciousEncodingSummary(opt)) => opt.output.as_ref(),
            Some(Action::ForensicReport(opt)) => opt.output.as_ref(),
            _ => None,
        };
        ...
        let validate_checksum = match &input_config.as_ref().unwrap().action {
            ...
            | Some(Action::AccountLockoutSummary(opt)) | Some(Action::PolicyTamperingSummary(opt))
            | Some(Action::RebootShutdownSummary(opt)) | Some(Action::SuspiciousEncodingSummary(opt)) => {
                opt.detect_common_options.validate_checksums
            }
            Some(Action::ForensicReport(opt)) => {
                opt.detect_common_options.validate_checksums
            }
        ...
        let mut ret = StoredStatic {
            ...
            forensic_summary_flag: (16..=41).contains(&action_id),
            forensic_report_flag: action_id == 42,
...
fn check_thread_number(config: &Config) -> Option<usize> {
    match config.action.as_ref()? {
        ...
        Action::ServiceSummary(opt) | Action::DriverSummary(opt) | Action::CrashSummary(opt)
        | Action::AccountChanges(opt) | Action::GroupChanges(opt) | Action::PrivilegeUseSummary(opt)
        | Action::PasswordChanges(opt) | Action::FirewallSummary(opt) | Action::RdpSummary(opt)
        | Action::ShareAccessSummary(opt) | Action::SoftwareInstallSummary(opt)
        | Action::WindowsUpdateSummary(opt) | Action::KerberosSummary(opt)
        | Action::FailedLogonDetail(opt) | Action::LogonTypeBreakdown(opt)
        | Action::AuditPolicyChanges(opt) | Action::LogCleared(opt)
        | Action::ObjectAccessSummary(opt) | Action::ScheduledTaskSummary(opt)
        | Action::ProcessExecutionSummary(opt) | Action::ScheduledPersistenceSummary(opt)
        | Action::LateralMovementSummary(opt) | Action::AccountLockoutSummary(opt)
        | Action::PolicyTamperingSummary(opt) | Action::RebootShutdownSummary(opt)
        | Action::SuspiciousEncodingSummary(opt) => opt.detect_common_options.thread_number,
        Action::ForensicReport(opt) => opt.detect_common_options.thread_number,
        _ => None,
    }
}
...
pub enum Action {
    ...
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe service-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 456, disable_help_flag = true)]
    /// Output a summary of service activity (install/change/crash)
    ServiceSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe driver-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 303, disable_help_flag = true)]
    /// Output a summary of kernel driver installations
    DriverSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe crash-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 293, disable_help_flag = true)]
    /// Output a summary of application crashes
    CrashSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe account-changes <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 270, disable_help_flag = true)]
    /// Output a summary of account lifecycle changes
    AccountChanges(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe group-changes <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 351, disable_help_flag = true)]
    /// Output a summary of group membership changes
    GroupChanges(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe privilege-use-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 421, disable_help_flag = true)]
    /// Output a summary of sensitive privilege usage
    PrivilegeUseSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe password-changes <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 419, disable_help_flag = true)]
    /// Output a summary of password change events
    PasswordChanges(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe firewall-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 320, disable_help_flag = true)]
    /// Output a summary of Windows Filtering Platform events
    FirewallSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe rdp-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 435, disable_help_flag = true)]
    /// Output a dedicated RDP session summary
    RdpSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe share-access-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 452, disable_help_flag = true)]
    /// Output a summary of network share access
    ShareAccessSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe software-install-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 455, disable_help_flag = true)]
    /// Output a summary of software installation events
    SoftwareInstallSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe windows-update-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 480, disable_help_flag = true)]
    /// Output a summary of Windows Update events
    WindowsUpdateSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe kerberos-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 370, disable_help_flag = true)]
    /// Output a summary of Kerberos authentication events
    KerberosSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe failed-logon-detail <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 319, disable_help_flag = true)]
    /// Output a detailed breakdown of failed logons by reason
    FailedLogonDetail(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe logon-type-breakdown <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 384, disable_help_flag = true)]
    /// Output a breakdown of logon types
    LogonTypeBreakdown(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe audit-policy-changes <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 275, disable_help_flag = true)]
    /// Output a summary of audit policy changes
    AuditPolicyChanges(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe log-cleared <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 381, disable_help_flag = true)]
    /// Output a summary of log clearing events
    LogCleared(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe object-access-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 405, disable_help_flag = true)]
    /// Output a summary of object access events
    ObjectAccessSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe scheduled-task-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 453, disable_help_flag = true)]
    /// Output a summary of scheduled task activity
    ScheduledTaskSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe process-execution-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 420, disable_help_flag = true)]
    /// Output a summary of process execution activity and save a DOT graph beside CSV output when -o is used
    ProcessExecutionSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe scheduled-persistence-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 454, disable_help_flag = true)]
    /// Output a summary of scheduled-task persistence changes
    ScheduledPersistenceSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe lateral-movement-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 379, disable_help_flag = true)]
    /// Output a summary of likely lateral movement activity
    LateralMovementSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe account-lockout-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 271, disable_help_flag = true)]
    /// Output a summary of account lockout events
    AccountLockoutSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe policy-tampering-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 422, disable_help_flag = true)]
    /// Output a summary of policy and logging tampering events
    PolicyTamperingSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe reboot-shutdown-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 436, disable_help_flag = true)]
    /// Output a summary of reboot and shutdown events
    RebootShutdownSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe suspicious-encoding-summary <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 458, disable_help_flag = true)]
    /// Output a summary of suspicious encoded command activity
    SuspiciousEncodingSummary(ForensicSummaryOption),
    #[clap(author = "", help_template = "\nForensic Analyzer v1.0.0\n{author-with-newline}\n{usage-heading}\n  hayabusa.exe forensic-report <INPUT> [OPTIONS]\n\n{all-args}", term_width = 400, display_order = 330, disable_help_flag = true)]
    /// Run all forensic summaries and generate a combined HTML report
    ForensicReport(ForensicReportOption),
}
...
impl Action {
    pub fn to_usize(action: Option<&Action>) -> usize {
        if let Some(a) = action {
            match a {
                ...
                Action::ServiceSummary(_) => 16,
                Action::DriverSummary(_) => 17,
                Action::CrashSummary(_) => 18,
                Action::AccountChanges(_) => 19,
                Action::GroupChanges(_) => 20,
                Action::PrivilegeUseSummary(_) => 21,
                Action::PasswordChanges(_) => 22,
                Action::FirewallSummary(_) => 23,
                Action::RdpSummary(_) => 24,
                Action::ShareAccessSummary(_) => 25,
                Action::SoftwareInstallSummary(_) => 26,
                Action::WindowsUpdateSummary(_) => 27,
                Action::KerberosSummary(_) => 28,
                Action::FailedLogonDetail(_) => 29,
                Action::LogonTypeBreakdown(_) => 30,
                Action::AuditPolicyChanges(_) => 31,
                Action::LogCleared(_) => 32,
                Action::ObjectAccessSummary(_) => 33,
                Action::ScheduledTaskSummary(_) => 34,
                Action::ProcessExecutionSummary(_) => 35,
                Action::ScheduledPersistenceSummary(_) => 36,
                Action::LateralMovementSummary(_) => 37,
                Action::AccountLockoutSummary(_) => 38,
                Action::PolicyTamperingSummary(_) => 39,
                Action::RebootShutdownSummary(_) => 40,
                Action::SuspiciousEncodingSummary(_) => 41,
                Action::ForensicReport(_) => 42,
            }
        } else {
            100
        }
    ...
    pub fn get_action_name(action: Option<&Action>) -> &str {
        if let Some(a) = action {
            match a {
                ...
                Action::ServiceSummary(_) => "service-summary",
                Action::DriverSummary(_) => "driver-summary",
                Action::CrashSummary(_) => "crash-summary",
                Action::AccountChanges(_) => "account-changes",
                Action::GroupChanges(_) => "group-changes",
                Action::PrivilegeUseSummary(_) => "privilege-use-summary",
                Action::PasswordChanges(_) => "password-changes",
                Action::FirewallSummary(_) => "firewall-summary",
                Action::RdpSummary(_) => "rdp-summary",
                Action::ShareAccessSummary(_) => "share-access-summary",
                Action::SoftwareInstallSummary(_) => "software-install-summary",
                Action::WindowsUpdateSummary(_) => "windows-update-summary",
                Action::KerberosSummary(_) => "kerberos-summary",
                Action::FailedLogonDetail(_) => "failed-logon-detail",
                Action::LogonTypeBreakdown(_) => "logon-type-breakdown",
                Action::AuditPolicyChanges(_) => "audit-policy-changes",
                Action::LogCleared(_) => "log-cleared",
                Action::ObjectAccessSummary(_) => "object-access-summary",
                Action::ScheduledTaskSummary(_) => "scheduled-task-summary",
                Action::ProcessExecutionSummary(_) => "process-execution-summary",
                Action::ScheduledPersistenceSummary(_) => "scheduled-persistence-summary",
                Action::LateralMovementSummary(_) => "lateral-movement-summary",
                Action::AccountLockoutSummary(_) => "account-lockout-summary",
                Action::PolicyTamperingSummary(_) => "policy-tampering-summary",
                Action::RebootShutdownSummary(_) => "reboot-shutdown-summary",
                Action::SuspiciousEncodingSummary(_) => "suspicious-encoding-summary",
                Action::ForensicReport(_) => "forensic-report",
            }
        } else {
            ""
        }
    ...
    /// Returns true if the action is one of our forensic summary commands.
    pub fn is_forensic_summary(action: Option<&Action>) -> bool {
        (16..=41).contains(&Self::to_usize(action))
    }
}
...
/// Shared option struct for all 18 forensic summary commands.
#[derive(Args, Clone, Debug, Default)]
pub struct ForensicSummaryOption {
    #[clap(flatten)]
    pub input_args: InputOption,

    /// Save results to CSV (ex: -o results.csv)
    #[arg(help_heading = Some("Output"), short = 'o', long, value_name = "FILE", display_order = 410)]
    pub output: Option<PathBuf>,
...
#[derive(Args, Clone, Debug, Default)]
pub struct ForensicReportOption {
    #[clap(flatten)]
    pub input_args: InputOption,

    /// Save HTML report to file (ex: -o report.html)
    #[arg(help_heading = Some("Output"), short = 'o', long, value_name = "FILE", display_order = 410)]
    pub output: Option<PathBuf>,
...
fn extract_output_options(config: &Config) -> Option<OutputOption> {
    match &config.action.as_ref()? {
        ...
        Action::ServiceSummary(option) | Action::DriverSummary(option)
        | Action::CrashSummary(option) | Action::AccountChanges(option)
        | Action::GroupChanges(option) | Action::PrivilegeUseSummary(option)
        | Action::PasswordChanges(option) | Action::FirewallSummary(option)
        | Action::RdpSummary(option) | Action::ShareAccessSummary(option)
        | Action::SoftwareInstallSummary(option) | Action::WindowsUpdateSummary(option)
        | Action::KerberosSummary(option) | Action::FailedLogonDetail(option)
        | Action::LogonTypeBreakdown(option) | Action::AuditPolicyChanges(option)
        | Action::LogCleared(option) | Action::ObjectAccessSummary(option)
        | Action::ScheduledTaskSummary(option) | Action::ProcessExecutionSummary(option)
        | Action::ScheduledPersistenceSummary(option) | Action::LateralMovementSummary(option)
        | Action::AccountLockoutSummary(option) | Action::PolicyTamperingSummary(option)
        | Action::RebootShutdownSummary(option) | Action::SuspiciousEncodingSummary(option) => Some(OutputOption {
            input_args: option.input_args.clone(),
            time_format_options: option.time_format_options.clone(),
            common_options: option.common_options,
            detect_common_options: option.detect_common_options.clone(),
        ...
        Action::ForensicReport(option) => Some(OutputOption {
            input_args: option.input_args.clone(),
            time_format_options: option.time_format_options.clone(),
            common_options: option.common_options,
            detect_common_options: option.detect_common_options.clone(),
